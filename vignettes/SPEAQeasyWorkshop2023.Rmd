...
ToDO Renee
Use same colors for sample variables and QC metrics to make plots consistent
...
In pups, the overall mapping rate was slightly and negatively correlated with the ribosomal rate, but again controls and experimental samples are equally distributed and the <mark style= "background-color: #FCF3CF"> <span style="font-family: monospace"> rRNA_rate</span> </mark> is pretty slow for all of them. 
...
Overall mapping rate is also positively correlated with plate and flowcell, relationships mainly given by samples from the 1st plate with low <mark style= "background-color: #FCF3CF"> <span style="font-family: monospace"> overallMapRate</span> </mark>, most of which were also in the 1st flowcell.
...
Basically what it does is to calculate the sum of squares of each variable and the sum of squares of the total model fit, summarizing the contribution of each variable in terms of the fraction of variation explained (FVE). Since it calculates the fraction of total variation attributable to each aspect of the study design, these fractions naturally sum to 1 [3].
...
<p class="comment">
❓***What are random and fixed effects? How to determine if a variable is one or the other?*** **Categorical variables** are usually modeled as **random effects**, i.e., non-measurable traits (something like 'randomly chosen or selected from a population') such as the batch, sex, flowcell, plate, individual, variables whose specific levels are not of particular interest, only the grouping of the samples by those variables. These are control variables/factors that vary randomly across individuals or groups and we use them because we must control for these effects.
**Continuous variables** must be modeled as **fixed effects**, they cannot be modeled as random effects. These correspond to variables that can be measured somehow and whose levels are themselves of interest (the QC metrics, for instance).
</p>
...
$\hat y =\sum_{1}^j\beta_{j}X_{j} + \sum_{1}^k\alpha_{k}Z_{k}+ \epsilon$
  ← the predicted expression of a gene across all samples is given by the samples' values in the $j$ fixed effects and $k$ random effects.  So basically the gene expression in the sample $i$ is given by $\hat y_i =\sum_{1}^j\beta_{j}X_{ij} + \sum_{1}^k\alpha_{k}Z_{ik}+ \epsilon_i$.
...
$$
X_j\beta_j= \ \ _{n \ \ samples}{\begin{bmatrix} _{j^{th}\ \ fixed\ \ effect }\\ X_{1j} \\ ... \\ X_{(n-1)j} \\ X_{nj}  \end{bmatrix}}\beta_j = {\begin{bmatrix}  X_{1j}\beta_j \\ ... \\ X_{(n-1)j}\beta_j \\ X_{nj} \beta_j \end{bmatrix}}
$$
...
- The variance of the $j$th fixed effect is  <mark style= "background-color: #FEF5E7">$\sigma_{\beta_j}^2=var({\beta_j}X_j)$</mark>

For random effects the variances are computed by variance component estimates with `VarCorr()` from `nmle`:

- The variance of the $k$th random effect is  <mark style= "background-color: #D6EAF8">$\sigma_{\alpha_k}^2=var({\alpha_k}Z_k)$</mark>

After that it calculates the variance of the predicted expression values $\sum_{i=1}^n(\hat y_i - \bar {\hat y})^2=var(\hat y)(n-1)$, where $\hat y_i = \sum_{1}^j\beta_{j}X_{ij} + \sum_{1}^k\alpha_{k}Z_{ik} + \epsilon_i$, considering all variables in the model:

- The total variance considering all sample variables is $var(\hat y)= \sigma_{Total}^2= var(X_{1}\beta_1)+var(X_{2}\beta_2)+...+var(X_{j}\beta_j)+var(Z_{1}\alpha_1)+var(Z_{2}\alpha_2)+...+var(Z_{k}\alpha_k)+var(\epsilon)=\sum_1^jvar(\beta_jX_j)+\sum_1^kvar(\alpha_kZ_k)+var(\epsilon)=$
<mark style= "background-color: #F9EBEA">$\sigma_{Total}^2=\sum_1^j{ \sigma_{\beta_j}^2} + \sum_1^k{ \sigma_{\alpha_k}^2} + \sigma_{\epsilon}^2$</mark>

- So the fraction of variance explained by the $j$th fixed effect is  <mark style= "background-color: ##FEF5E7"> $\sigma_{\beta_j}^2$</mark> /   <mark style= "background-color: #F9EBEA">$\sigma_{Total}^2$</mark>
- The fraction of variance explained by the $k$th random effect is  <mark style= "background-color: #D6EAF8">$\sigma_{\alpha_k}^2$</mark> /    <mark style= "background-color: #F9EBEA">$\sigma_{Total}^2$</mark>
- The residual variance is  <mark style= "background-color: #E8DAEF">$\sigma_{\epsilon}^2$</mark> /    <mark style= "background-color: #F9EBEA">$\sigma_{Total}^2$</mark>, i.e., the variance that the model wasn't able to explain. Think of the residuals as $\hat y_i - y_i$ (the difference between the predicted expression and the real expression observed).
    
</aside>


<aside>
3️⃣ Finally, it obtains $\frac{\sigma_{V_h}^2}{\sigma_{Total}^2\times \sum{\sigma_{V_m}^2}}$, which  can be thought as the variance explained by the variable $h$ (either random or fixed) over the variance explained by the model with all the variables, scaled by the sum of the variances of the $m$ variables of the study. All those values are divided by their sum so that they sum 1 for a single gene. These fractions are always positive.

</aside>

* Source code of `calcVarPart()`: https://rdrr.io/github/GabrielHoffman/variancePartition/src/R/calcVarPart.R#sym-getVarianceComponents
...
plot + theme(plot.margin=unit (c(1,1,1,1), 'cm'), 
              ...
              axis.text.y = element_text(size = (7.5)))
```
Notwithstanding, in this new reduced model Group contribution doesn't increment much in comparison with the complete model and also notice that the % of variance explained by the residuals, i.e., the % of gene expression variance that the model couldn't explain, increments in this model compared to the previous. That's the price to pay when dropping variables but it is convenient in this case since we don't have many samples for the model to determine their real unique contributions.
...
At this point we have extracted normalized counts and filtered genes, we have filtered poor-quality samples and we have decided which sample variables to retain for future analyses. Now we are ready for the DEA!
