...
* The part of variance explained by a biologically relevant variable can be reduced by correlated variables' apparent contributions, if for example, they contain very similar information.
...
As shown above and as previously demonstrated, the mitochondrial rate and the fraction of reads that mapped to genes are negatively correlated in adults but control and cases are evenly distributed. 
...
In pups, the overall mapping rate was slightly and negatively correlated with the ribosomal rate, but again controls and cases are equally distributed and the <mark style= "background-color: #FCF3CF"> <span style="font-family: monospace"> rRNA_rate</span> </mark> is pretty slow for all of them. 
...
Overall mapping rate is also positively correlated with plate and flowcell, relationships mainly given by samples from the 1st plate with low <mark style= "background-color: #FCF3CF"> <span style="font-family: monospace"> overallMapRate</span> </mark>, most of them were also in the 1st flowcell.
...
<div class="alert alert-info">
Now look at the following plots. **Why is it important that cases and controls are distributed throughout all the plates and flowcells?** 
```{r message=FALSE, warning=FALSE}
...
```
***Hint***: What would happen if all cases were in one plate or flowcell and controls in another?
</div>

After identifying which variables are correlated and exploring why, the next step is to determine which of these correlated variables must be removed. 

**How to discern which variables to keep and which to drop?**

As recommended in the `variancePartition` user’s guide [4], initially we can fit a linear model to each gene taking all sample variables and then define which ones explain a higher percentage of variance in many genes.
