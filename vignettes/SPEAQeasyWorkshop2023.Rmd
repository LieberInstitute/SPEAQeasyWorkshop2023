...
This workshop aims to present the `SPEAQeasy` `r Citep(bib[["SPEAQeasy"]])` RNA-seq processing pipeline, demonstrating how its outputs can be used in Differential Expression Analyses (DEA) and the type of results it enables to obtain using other Bioconductor R packages such as `r Biocpkg('limma')` and `r Biocpkg('edgeR')`.


## Description
In this demo, participants will be able to understand what the SPEAQeasy pipeline does and what it returns through a differential expression analysis performed on a dataset from the `smokingMouse` `r Citep(bib[["smokingMouse"]])` Bioconductor package, which contains gene expression data and sample information from multiple RNA-sequencing experiments in mice. 
...
## Participation
Participants are expected to follow us during the theoretical explanations and while presenting R code to create plots and perform analyses so that they can run code in their own computers and extract their own results. 
...
| Data overview                           | 10m |
| Differential Expression Analysis        | 65m |
...
* to understand how to identify suitable covariates for differential expression analyses;
...
* use `r Biocpkg('variancePartition')` to perform a Canonical Correlation Analysis (CCA) on gene-expression data with associated quality metrics;
* to use `r Biocpkg('limma')` functions such as `voom()`, `lmFit()`, `eBayes()` and `topTable()` to find DEGs. 
* to use `r Biocpkg('ComplexHeatmap')` to create heatmaps of expression levels of DEGs, identifying clusters of up and down regulated genes. 
...
The dataset that will be used for the DEA can be downloaded from the `smokingMouse` package. We will download the `rse_gene` object which contains the data at the gene level. Visit [here](http://research.libd.org/smokingMouse/) for more details of this package.
